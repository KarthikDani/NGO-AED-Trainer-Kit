...
def amplify_audio(audio_path: str, amplification_db: int) -> AudioSegment:
    """Increases the volume of the audio file by a given decibel value."""
...
def process_language_audio(language_dir: str, beat_path: str, output_dir: str, 
                           amplification_languages: list[str], amplification_db: int = 10) -> None:
    # Load the beat audio
        ...
        if os.path.isdir(lang_path):
            ...
            if shock_path and no_shock_path:
                ...
                for order, audio in sequences.items():
                    ...
                    audio.set_frame_rate(16000).export(output_path_main, format="wav")
                    audio.set_frame_rate(16000).export(output_path_sub, format="wav")
...
amplification_languages = ["telugu", "tamil", "malayalam", "english"]
